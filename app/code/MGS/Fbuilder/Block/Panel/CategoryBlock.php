 ...
 */
class CategoryBlock extends \MGS\Fbuilder\Block\Panel\Block
{
    public function getBlocks()
    {
        ...
        $blocks = $this->_childCollectionFactory->create()
            ...
            ->addFieldToFilter('product_id', $category->getId())
            ->setOrder('position', 'ASC');
        if ($this->getOverriden()==1) {
    ...
    public function getEditChildHtml($block, $child)
    {
        ...
        $html .= '<li><a href="' . $this->getUrl('fbuilder/create/element', ['product_id'=>$child->getProductId(), 'page_type'=>'category', 'overriden'=>$child->getStoreId(), 'block' => $block, 'id' => $child->getId(), 'type' => $child->getType()]) . '" class="popup-link" title="' . __('Edit') . '"><em class="fa fa-edit">&nbsp;</em></a></li>';
