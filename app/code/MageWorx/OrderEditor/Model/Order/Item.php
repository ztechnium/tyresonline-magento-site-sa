        ...
        foreach ($productCustomOptions as $option) {
            ...
            if ($optionsFromRequest !== null && isset($optionsFromRequest[$option->getId()])) {
                $optionValueForFormatter = is_array($optionsFromRequest[$option->getId()]) ?
                    $this->serializerJson->serialize($optionsFromRequest[$option->getId()]) :
                    $optionsFromRequest[$option->getId()];
                $formattedValue          = $group->getFormattedOptionValue($optionValueForFormatter);
                $valueDirect             = $this->serializerJson->unserialize($quoteItemOptionObject->getValue());

                if (!empty($valueDirect)) { // Do not display empty options in product info block
                    $optionData = [
                        'label'        => $option->getTitle(),
                        'value'        => $formattedValue,
                        'print_value'  => $group->getPrintableOptionValue($quoteItemOptionObject->getValue()),
                        'option_id'    => $option->getId(),
                        'option_type'  => $option->getType(),
                        'option_value' => $this->serializerJson->serialize($valueDirect),
                        'custom_view'  => $group->isCustomizedView()
                    ];

                    $results[]                                  = $optionData;
                    $buyRequestOptionsForSave[$option->getId()] = $optionData;
                }
            }
        }

        $buyRequest->setOptions($buyRequestOptionsForSave);
