...
use Lof\AdvancedReports\Api\ProductsreportInterface;
use Lof\AdvancedReports\Model\AbstractReport;
use Magento\Framework\Api\SortOrder;
 
class ProductsreportRepository extends AbstractReport implements ProductsreportInterface
{
    protected $_limit = 10;
    /**
     * GROUP BY criteria
    ...
    public function __construct(   
        ...
        \Lof\AdvancedReports\Api\Data\ProductsreportdataInterfaceFactory $searchResultsFactory
        )
    {
    ...
    */
     ...
     * @return \Lof\AdvancedReports\Api\Data\ProductsreportdataInterface
     */
    ...
    protected function _convertGridData(&$collection = null) {
        ...
        if($collection && $collection->getSize()) {
            ...
            foreach($collection as &$item) {
                ...
                $avg_price_currency = $this->formatCurrency($item->getAvgPrice());
                $item->setData("avg_price_currency", $avg_price_currency);
                ...
                $avg_revenue_currency = $this->formatCurrency($item->getAvgRevenue());
                $item->setData("avg_revenue_currency", $avg_revenue_currency);

                $avg_discount_amount = $this->formatCurrency($item->getAvgDiscountAmount());
                $item->setData("avg_discount_amount", $avg_discount_amount);

                $avg_tax_currency = $this->formatCurrency($item->getAvgTaxAmount());
                $item->setData("avg_tax_currency", $avg_tax_currency);

                $avg_refunded_currency = $this->formatCurrency($item->getAvgRefundedAmount());
                $item->setData("avg_refunded_currency", $avg_refunded_currency);
            }
        }
        return $collection;
    }
