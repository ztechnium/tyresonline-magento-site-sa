...
class  Collection extends \Lof\AdvancedReports\Model\ResourceModel\AbstractReport\Ordercollection
{
    ...
    public function prepareOverviewCollection() {
     ...
     public function prepareVehcileCollection() {
  
        $this->_aggregateByVehcileFields();
        return $this;
    }
     public function prepareSalespersonCollection() {
  
        $this->_aggregateBySalespersonFields();
        return $this;
    }
     public function prepareInstallerCollection() {
  
        $this->_aggregateByInstallerFields();
        return $this;
    }
    public function prepareMakeModelCollection() {
  
        $this->_aggregateByMakeModelFields();
        return $this;
    }
...
/**
    ...
    protected function _aggregateByField($aggregationField = "", $hide_fields = array(), $show_fields = array())
    {
     ...
     protected function _aggregateByVehcileFields($aggregationField = "", $hide_fields = array(), $show_fields = array())
    {
        $adapter = $this->getResource()->getConnection();  
        try {
            ...
            $columns = array(
             ...
             //   'product_type'                   => 'oi.product_type',
                'orders_count'                   => new \Zend_Db_Expr('COUNT(main_table.entity_id)'),
                'uniqu_orders_count'                   => new \Zend_Db_Expr('COUNT(DISTINCT(main_table.entity_id))'),
                'total_qty_ordered'              => new \Zend_Db_Expr('SUM(oi.total_qty_ordered)'),
                'total_subtotal_amount'          => new \Zend_Db_Expr('SUM(main_table.subtotal)'),
             //   'total_qty_invoiced'             => new \Zend_Db_Expr('SUM(oi.total_qty_invoiced)'),
                'total_grandtotal_amount'        => new \Zend_Db_Expr('SUM(main_table.grand_total)'),
                'total_pograndtotal_amount'        => new \Zend_Db_Expr('SUM(main_table.po_grandtotal)'),
                 'margin'        => new \Zend_Db_Expr('SUM(main_table.po_margin)'),
                'po_marginperc'        => new \Zend_Db_Expr('SUM(main_table.po_marginperc)')
            ...
            $this->getSelect()->columns($columns)
             ...
             //$this->getSelect()->join(array('si' => 'sales_invoice'), 'si.order_id = main_table.entity_id', array());     
                $this->getSelect()->join(array('si' => 'sales_invoice'), 'si.order_id = main_table.entity_id',
               array('total_qty',"sum(si.total_qty) as total_qtyty",'grand_total',"sum(si.grand_total) as inv_grand_total"
           //    'margin' => new \Zend_Db_Expr('main_table.grand_total - main_table.po_grandtotal')
           ));
     ...
     protected function _aggregateBySalespersonFields($aggregationField = "", $hide_fields = array(), $show_fields = array())
    {
        $adapter = $this->getResource()->getConnection();  
        try {
            ...
            $columns = array(
                ...
                'store_id'                       => 'main_table.store_id',
                'order_status'                   => 'main_table.status',
                'sales_person_id'                   => 'main_table.sales_person_id',
             //   'product_type'                   => 'oi.product_type',
                'orders_count'                   => new \Zend_Db_Expr('COUNT(main_table.entity_id)'),
                'uniqu_orders_count'                   => new \Zend_Db_Expr('COUNT(DISTINCT(main_table.entity_id))'),
                'total_qty_ordered'              => new \Zend_Db_Expr('SUM(oi.total_qty_ordered)'),
            ...
            $this->getSelect()->columns($columns)
             ...
             $this->getSelect()->join(array('si' => 'sales_invoice'), 'si.order_id = main_table.entity_id',
               array('total_qty',"sum(si.total_qty) as total_qtyty",'grand_total',"sum(si.grand_total) as inv_grand_total"
                //'margin' => new \Zend_Db_Expr('SUM(main_table.grand_total) - SUM(main_table.po_grandtotal)')
               // 'margin' => new \Zend_Db_Expr('main_table.grand_total - main_table.po_grandtotal')
           ));

             // add po total

             // $this->getSelect()->joinLeft(array('po' => 'purchase_order'), 'po.orderreference_no = main_table.increment_id',
             //   array("sum(po.grandtotal) as grandtotals"));

             $this->getSelect()->join(array('au' => 'admin_user'), 'au.user_id = main_table.sales_person_id', 
                array('adminname'=>"CONCAT(firstname, ' ', lastname)"));     
                ...
                //     \Magento\Sales\Model\Order::STATE_NEW
                // ));
              $this->getSelect()->group('sales_person_id');
       ...
       protected function _aggregateByInstallerFields($aggregationField = "", $hide_fields = array(), $show_fields = array())
    {
        ...
        try {
            ...
            $columns = array(
                ...
                'store_id'                       => 'main_table.store_id',
                'order_status'                   => 'main_table.status',
                'installer_id'                   => 'main_table.installer_id',
             //   'product_type'                   => 'oi.product_type',
                'orders_count'                   => new \Zend_Db_Expr('COUNT(main_table.entity_id)'),
                'uniqu_orders_count'                   => new \Zend_Db_Expr('COUNT(DISTINCT(main_table.entity_id))'),
                'total_qty_ordered'              => new \Zend_Db_Expr('SUM(oi.total_qty_ordered)'),
            ...
            $this->getSelect()->columns($columns)
             ...
             //   array("sum(po.grandtotal) as grandtotals"));
              ...
              $this->getSelect()->join(array('sitem' => 'storelocator_item'), 'sitem.item_id = main_table.installer_id', 
                array('sitem.name as installername'));     
                ...
                //     \Magento\Sales\Model\Order::STATE_NEW
                // ));
              $this->getSelect()->group('installer_id');
      ...
      protected function _aggregateByMakeModelFields($aggregationField = "", $hide_fields = array(), $show_fields = array())
    {
        $adapter = $this->getResource()->getConnection();  
        try {
