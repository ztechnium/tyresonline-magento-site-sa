...
namespace Lof\AdvancedReports\Model\ResourceModel\Customer\Customer;
...
{   
 ...
 public function getDateColumnFilter() {
    ...
    public function prepareNewAccountsCollection($group_by = "") {
        $hide_fields = array();
        $this->setMainTable('customer_entity');
        $this->_aggregateNewAccountsByField($group_by, $hide_fields);
        return $this;
    }
     ...
     */
    protected function _aggregateNewAccountsByField($aggregationField = "", $hide_fields = array(), $show_fields = array())
    {
        ...
        try {
            ...
            $this->getSelect()->columns($columns)
                            ->join(array('oadd' => $this->getTable('sales_order_address')), 'main_table.entity_id = oadd.customer_id', array("region","postcode","street","city","country_id"))
                            ->where('oadd.customer_id IS NOT NULL');
