...
use Lof\AdvancedReports\Api\OrderdetailInterface;
use Lof\AdvancedReports\Model\AbstractReport;
use Magento\Framework\Api\SortOrder;
 
class OrderdetailRepository extends AbstractReport implements OrderdetailInterface
{
    protected $_limit = 10;
    /**
     * GROUP BY criteria
    ...
    public function __construct(   
        ...
        \Lof\AdvancedReports\Api\Data\OrderdetaildataInterfaceFactory $searchResultsFactory
        )
    {
    ...
    */
    public function initFilterData($filter_field = []) {
        ...
        $margin = $this->_helperData->getConfig("charts_settings/margin");
    ...
    public function getOrderDetail(\Magento\Framework\Api\SearchCriteriaInterface $searchCriteria) {
        ...
        $resourceCollection = $this->_objectManager->create($this->getResourceCollectionName())
            ->prepareOrderDetailedCollection($filterData->getData('margin_profit', 0))
    ...
    protected function _convertGridData(&$collection = null) {
        ...
        if($collection && $collection->getSize()) {
            ...
            foreach($collection as &$item) {
                ...
                $total_subtotal_amount = $this->formatCurrency($item->getTotalSubtotalAmount());
                $item->setData("total_subtotal_amount_currency", $total_subtotal_amount);

                $total_grossprofit_amount = $this->formatCurrency($item->getTotalGrossprofitAmount());
                $item->setData("total_grossprofit_amount_currency", $total_grossprofit_amount);

                $margin_profit_amount = $this->formatCurrency($item->getMarginProfitAmount());
                $item->setData("margin_profit_amount_currency", $margin_profit_amount);

                $total_net_profits = $this->formatCurrency($item->getTotalNetProfitsAmount());
                $item->setData("total_net_profits_currency", $total_net_profits);
