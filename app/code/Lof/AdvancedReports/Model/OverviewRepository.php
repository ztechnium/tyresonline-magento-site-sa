...
use Lof\AdvancedReports\Api\OverviewInterface;
use Lof\AdvancedReports\Model\AbstractReport;
use Magento\Framework\Api\SortOrder;
 
class OverviewRepository extends AbstractReport implements OverviewInterface
{
    protected $overviewResultsFactory;
    protected $_limit = 10;
    /**
     * GROUP BY criteria
     *
    ...
    public function __construct(   
        ...
        \Lof\AdvancedReports\Api\Data\OverviewdataInterfaceFactory $overviewResultsFactory
        )
    {
        $this->overviewResultsFactory = $overviewResultsFactory;
    ...
    */
     ...
     * @return \Lof\AdvancedReports\Api\Data\OverviewdataInterface
     */
    public function getSalesOverview(\Magento\Framework\Api\SearchCriteriaInterface $searchCriteria) {
        $cur_month = date("m");
        $cur_year = date("Y");
        $filter_fields = [
        ...
        $resourceCollection = $this->_objectManager->create($this->getResourceCollectionName())
            ...
            ->setDateColumnFilter($filterData->getData('report_field', $this->_columnDate))
        ...
        //init \Lof\AdvancedReports\Api\Data\OverviewdataInterface
        $searchResult = $this->overviewResultsFactory->create();
    ...
    protected function _convertGridData(&$collection = null) {
        ...
        if($collection && $collection->getSize()) {
            ...
            foreach($collection as &$item) {
                ...
                $subtotal_currency = $this->formatCurrency($item->getTotalSubtotalAmount());
                $item->setData("subtotal_currency", $subtotal_currency);

                $tax_currency = $this->formatCurrency($item->getTotalTaxAmount());
                $item->setData("tax_currency", $tax_currency);

                $shipping_currency = $this->formatCurrency($item->getTotalShippingAmount());
                $item->setData("shipping_currency", $shipping_currency);

                $discount_currency = $this->formatCurrency($item->getTotalDiscountAmount());
                $item->setData("discount_currency", $discount_currency);

                $grandtotal_currency = $this->formatCurrency($item->getTotalGrandtotalAmount());
                $item->setData("grandtotal_currency", $grandtotal_currency);

                $invoiced_currency = $this->formatCurrency($item->getTotalInvoicedAmount());
                $item->setData("invoiced_currency", $invoiced_currency);

                $refunded_currency = $this->formatCurrency($item->getTotalRefundedAmount());
                $item->setData("refunded_currency", $refunded_currency);
