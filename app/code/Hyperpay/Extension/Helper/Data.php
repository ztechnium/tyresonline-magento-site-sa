...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    if(isset($shippingAddress) && !empty($shippingAddress)){
        ...
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/vky.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        $logger->info('Hyperpay\Extension\Helper Data');
        $logger->info(print_r(
                    $data
                , true));
	...
	public function getPayableAmount()
    {
		...
		$grandTotal = $payment->formatPrice($amount);
		return $grandTotal;
	}
}
