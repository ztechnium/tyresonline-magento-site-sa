...
define([
    ...
    return function (config) {
        var attributeOption   = {
                ...
                renderWithDelay: function (element, data, from, step, delay) {
                    ...
                    data.each(function (value) {
                        $('select[name="groups[googletag][groups][ga4][fields][custom_' + element + '][value][option][value][' + value.id + '][value]"]').val(value.value);
                        $('select[name="groups[googletag][groups][ga4][fields][custom_' + element + '][value][option][value][' + value.id + '][index]"]').val(value.index);
                    });
                ...
                showCustomField: function () {
                    if ($('#googletagmanager_googletag_ga4_enabled_ga4').val() === '0') {
                        $('#row_mageplaza_gtm_connections').hide();
                        $('#row_mageplaza_gtm_dimensions').hide();
                        $('#row_mageplaza_gtm_metrics').hide();
                    } else {
                        $('#row_mageplaza_gtm_connections').show();
        ...
        attributeOption.showCustomField();

        $('#googletagmanager_googletag_ga4_enabled_ga4').change(function () {
            attributeOption.showCustomField();
        });
    };
});
