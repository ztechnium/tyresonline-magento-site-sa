...
class Barcode
{
    public static function draw($text, $width, $height)
    {
        $codeString = "";
        $chksum = 104;
        $codeArray = [
             ...
             "{"=>"412121", "|"=>"111143", "}"=>"111341", "~"=>"131141", "DEL"=>"114113", "FNC 3"=>"114311",
             "FNC 2"=>"411113", "SHIFT"=>"411311", "CODE C"=>"113141", "FNC 4"=>"114131", "CODE A"=>"311141",
             "FNC 1"=>"411131", "Start A"=>"211412", "Start B"=>"211214", "Start C"=>"211232", "Stop"=>"2331112"
        ];
        
        $codeKeys = array_keys($codeArray);
        $code_values = array_flip($codeKeys);
        $textLenght =  strlen($text);
        for ($x = 1; $x <= $textLenght; $x++) {
        ...
        $codeLength = 0;
        $stringLenght = strlen($codeString);
        for ($i = 1; $i <= $stringLenght; $i++) {
            $codeLength = $codeLength + (integer)(substr($codeString, ($i-1), 1));
        }
        
        $sizeFactory = ceil($width / $codeLength);
        $imgWidth = $codeLength * $sizeFactory;
        $imgWidth = ($imgWidth > $width)?$imgWidth:$width;
        
        $imgSizeRatio = $imgWidth / $width;
        $imgHeight = $height * $imgSizeRatio;
        $sizeFactory = $sizeFactory;
        
        $image = imagecreate((int)$imgWidth, (int)$imgHeight);
        ...
        for ($position = 1; $position <= $stringLenght; $position++) {
            $cur_size = $location + ( substr($codeString, ($position-1), 1) );
            $color = ($position % 2 == 0 ? $white : $black);
            imagefilledrectangle($image, $location * $sizeFactory, 0, $cur_size * $sizeFactory, (int)$imgHeight, $color);
            $location = $cur_size;
        }
