...
class MtEditorManager
{
    ...
    public function getAvailableVariableList($templateId, $sourceId)
    {
        ...
        $vehicleInfoKeys = array_keys($filter->addVehicleData([]));
        $installerInfoKeys = array_keys($filter->addInstallerData([]));
        $storeInfoKeys = array_keys($filter->addStoreInfoData([]));
        ...
        foreach ($variables as $key => $value) {
            ...
            }elseif (in_array($key, $vehicleInfoKeys)) {
                $groupedVariables['vehicle'][$key] = $value;
            }elseif (in_array($key, $installerInfoKeys)) {
                $groupedVariables['installer'][$key] = $value;
            }elseif (in_array($key, $storeInfoKeys)) {
                $groupedVariables['storeinfo'][$key] = $value;
            } else {
