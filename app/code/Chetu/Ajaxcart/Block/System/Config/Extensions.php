...
class Extensions extends \Magento\Config\Block\System\Config\Form\Fieldset
{
    ...
    public function render(\Magento\Framework\Data\Form\Element\AbstractElement $element)
    {
        ...
        foreach ($modules as $moduleName) {
            if (strstr($moduleName, 'Chetu_') === false) {
                continue;
            }
            if ($moduleName === 'Chetu_Ajaxcart') {
				$moduleName = 'Chetu_Ajaxcart';
    ...
    protected function _getFieldHtml($fieldset, $moduleCode)
    {
        ...
        $status = '<a  target="_blank"><img src="' . $this->getViewFileUrl('Chetu_Ajaxcart::images/click.gif') . '" title="' . __("Installed") . '"/></a>';
