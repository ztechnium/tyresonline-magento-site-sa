define([
    ...
    $.widget('chetu.ajaxToCart', $.chetu.ajaxSuite, {
        options: {
            ajaxCart: {
        ...
        initEvents: function () {
            ...
            $('body').delegate(self.options.ajaxCart.addToCartButtonSelector, 'click', function (e) {
                ...
                if ($(this).closest('.lubricants-product').length) {             //list page if lubricant product
                    e.preventDefault();
                    var form = $(this).closest('form');
                    self.ajaxSubmit(form);
                    return;
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.html_popup) {
                        ...
                        var errorMessageInterval = setInterval(function () {
                            var messageElm = $('.page.messages .message-error div[data-bind="html: $parent.prepareMessageForHtml(message.text)"]');
                            if (messageElm.length > 0 && messageElm.text() != '') {
                                clearInterval(errorMessageInterval);
                                self.options.cartWrapper.find('.error-message').text(messageElm.text());
                            }
