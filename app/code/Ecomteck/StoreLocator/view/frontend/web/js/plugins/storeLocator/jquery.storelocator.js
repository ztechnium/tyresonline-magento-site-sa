...
define(["jquery"], // Require jquery
	...
	if (document.getElementById('mobilevanservice_location_cart') !=null) {
		var searchInput = document.getElementById('mobilevanservice_location_cart');
		var autoPlaces = new google.maps.places.Autocomplete(searchInput);
		autoPlaces.setComponentRestrictions({'country': ['sa']});
		// Add listener when autoComplete selection changes.
		autoPlaces.addListener('place_changed', function(){
			var place = autoPlaces.getPlace();
			var mobilevanservice_location_searchlat=place.geometry.location.lat();
			var mobilevanservice_location_searchlong=place.geometry.location.lng();
			$('#mobilevanservice_location_lat').val(mobilevanservice_location_searchlat);
			$('#mobilevanservice_location_lng').val(mobilevanservice_location_searchlong);
			var inputAddress = $('#mobilevanservice_location_cart').val();
			$('#mobilevanservice_address').val(inputAddress);
		});
	}
	var searchlatSort;
	var searchlongSort;
	...
	$('#calculatedistace').click(function(){
		if(searchlongSort != "" && searchlatSort != "" && $('#tyinstaller').val() != "" ) {
			var p1 = new google.maps.LatLng(searchlatSort, searchlongSort);
			$('.installer').each(function(i, obj) {
				var p2 = new google.maps.LatLng($(this).attr('data-lat'), $(this).attr('data-lng'));
				...
				if(window.checkoutConfig.storeCode == 'en'){
					$(this).find('.miles').html(finalinstallerrdistance+' kms');
				}else{
					$(this).find('.miles').html(' كيلومترات '+finalinstallerrdistance);
				}
				$(this).find('.distance').show();
				var selectdistance=$('#serach_distance').val();
				$(this).removeAttr('data-position');
				$(this).attr('data-position',finalinstallerrdistance);
			...
			$('.installer').sort(function (a, b) {
				var contentA =parseInt( $(a).attr('data-position'));
				var contentB =parseInt( $(b).attr('data-position'));
				return (contentA < contentB) ? -1 : (contentA > contentB) ? 1 : 0;
			}).appendTo(".allInstaller");
			
			setTimeout(function(){ $('body').trigger('processStop'); }, 3000);
		}else{
			$('.installer').each(function(i, obj) {
			  $(this).show();
			  $(this).find('.distance').hide();
			  $(this).find('.miles').html();
			});
		}
	});
