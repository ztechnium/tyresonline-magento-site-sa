...
function(
  ...
  $("#tyresize-input-search").on("keyup", function() {
    ...
    $(".alltyresize #searchUL .li-search").filter(function() {
      var text = ($(this).text().toLowerCase() + $(this).data('lookup')).toLocaleLowerCase();
      $(this).toggle(text.indexOf(value) != -1);
    });
  });
  
  
  jQuery('.catalog-product-view .select_qty').on('select2:select', function (e) {
  ...
  jQuery('.select_qty_btm').on('select2:select', function (e) {
    var selectedQty=jQuery(this).val();
    jQuery('.regular-product-list .select_qty').val(selectedQty).trigger('change');
  });
   ...
   jQuery(document).on('change', '.regular-product-list .select_qty', function() {
         ...
         // var selectedPrice=jQuery(this).parent().parent().find('.single-tyre-price').text().replace(/,/g, '');
          var selectedPrice=jQuery(this).closest('.tyres-productset4').find('.single-tyre-price').text().replace(/,/g, '');
          ...
          if (partsCat == 'Tyres' || partsCat == 'Lubricants'){
                   ...
                   if(discountqty != undefined && discountqty != undefined) { 
                          ...
                          if(selectedQty >= 4 && offer_price_with_deducted_ammount > 0){
                            jQuery(this).closest('.tyres-productset4').find('.set4price').addClass('strike-price');
                            jQuery(this).closest('.tyres-productset4').find('.setfourprice').html(formatedActualTotal);
                            jQuery(this).closest('.tyres-productset4').find('.set4').html('Set of '+selectedQty+':');
							jQuery(this).closest('.tyres-productset4').find('.set4price').show();
							jQuery(this).closest('.tyres-productset4').find('.set2price .set2').hide();
                          }else{
                            jQuery(this).closest('.tyres-productset4').find('.set4price').removeClass('strike-price');
                            jQuery(this).closest('.tyres-productset4').find('.setfourprice').html(formatedSetFourPrice);
                            jQuery(this).closest('.tyres-productset4').find('.set4').html('Set of 4:');
							jQuery(this).closest('.tyres-productset4').find('.set4price').hide();
							jQuery(this).closest('.tyres-productset4').find('.set2price .set2').show();
                          }
                    }
                  ...
                  jQuery(this).closest('.tyres-productset4').find('.settwoprice').html(formatedTotal);
                  jQuery(this).closest('.tyres-productset4').find('.set2').html('Set of '+selectedQty+':');
              ...
              if(discountqty == undefined && discountqty == undefined) { 
                if(selectedQty == 4){
                 ...
                 // jQuery(this).closest('.tyres-productset4').find('.set4price').show();
                  if(selectedQty == 1){
                    jQuery(this).closest('.tyres-productset4').find('.set1price').hide();
					if(ruleDiscount != undefined){
						jQuery(this).closest('.tyres-productset4').find('.set1price').show();
					}
                  }
     ...
     if(parseFloat(simpleTotal) >= tabbyfourday && parseFloat(simpleTotal) <= tabbyinstallmentmaxvalue && istabbyAvailable == 'Yes'){
      jQuery(this).closest('.tyres-productset4').find('.tabby-pay-later-list').removeClass('hidetabbyinstaller');
      jQuery(".tabby-product-detail").show();
     }else{
      jQuery(this).closest('.tyres-productset4').find('.tabby-pay-later-list').addClass('hidetabbyinstaller');
     }
   ...
   jQuery(document).on('change', '.bundle-product-list .select_qty', function() {
           ...
           var  front_qty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.front-qty').val();
           var  front_price=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.leftbundle').find('.single-tyre-price').text().replace(/,/g, '');
           ...
           var discountqty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.front-qty').data('discountqty');  //  Discount Amount percen
           var totalqty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.front-qty').data('totalqty');  // means  Discount Qty Step (Buy X)
           var selected_qty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.front-qty').val();
           var ruleDiscountFront = jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.front-qty').data('rule-discount');
          //if(jQuery(this).hasClass('front-qty')){
			var formatedFrontTotal = formatPriceWithComma(frontTotal);  
			jQuery(this).parents('.bundleBox').find('.leftbundle').find('.setfourprice').html(formatedFrontTotal);
			jQuery(this).parents('.bundleBox').find('.leftbundle').find('.setOf').html('Set of '+front_qty+':');  
           
		   var unitprice=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.front-qty').data('unitprice');
           if(discountqty != undefined && discountqty != undefined) { 
				   ...
				   if(selected_qty >= 4 && offer_price_with_deducted_ammount > 0){
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.setfourprice').html(formatedActualPrice);
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set4price').addClass('strike-price');
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set4price').show();
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set2price .set2').hide();
				  }else{
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set4price').removeClass('strike-price');
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.setfourprice').html(formatedFrontTotal);
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set4price').hide();
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set2price .set2').show();
				  }
				  jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set2price .setfourprice').html(formatedFrontTotal);
              }
			  if(ruleDiscountFront != undefined) {		  
				 ...
				 if(selected_qty >= 4){
					...
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set4price').removeClass('strike-price');
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.setfourprice').html(formatedSetofPrice);
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set4price').hide();
					jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set2price .set2').show();
			  ...
			  if(discountqty == undefined && discountqty == undefined) { 
                if(selected_qty == 4){
                  ...
                  if(selected_qty == 1){
                    jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set1price').hide();
					if(ruleDiscountFront != undefined){
						jQuery(this).parents('.bundleBox').find('.leftbundle').find('.set1price').show();
					}
                  }
                }
              }
          //}  
           ...
           var  rear_qty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rear-qty').val();
           var  rear_price=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rightbundle').find('.single-tyre-price').text().replace(/,/g, '');
           ...
           var discountqty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rear-qty').data('discountqty');
           var totalqty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rear-qty').data('totalqty');
           var selected_qty=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rear-qty').val();
		   var ruleDiscountRear = jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rear-qty').data('rule-discount');
		   
		   var formatedRearTotal = formatPriceWithComma(rearTotal);
		   
		   jQuery(this).parents('.bundleBox').find('.rightbundle').find('.setfourprice').html(formatedRearTotal);
           jQuery(this).parents('.bundleBox').find('.rightbundle').find('.setOf').html('Set of '+rear_qty+':');
		   var unitprice=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find('.rear-qty').data('unitprice');
            // if(jQuery(this).hasClass('rear-qty')){
               if(discountqty != undefined && discountqty != undefined) { 
					  ...
					  actual_price=parseFloat(actual_price).toFixed(2);
					   ...
					   if(selected_qty >= 4 && offer_price_with_deducted_ammount > 0){
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.setfourprice').html(formatedActualPrice);
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set4price').addClass('strike-price');
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set4price').show();
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set2price .set2').hide();
					  }else{
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set4price').removeClass('strike-price');
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.setfourprice').html(formatedRearTotal);
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set4price').hide();
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set2price .set2').show();
					  }
					  jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set2price .setfourprice').html(formatedRearTotal);
                }
				...
				if(ruleDiscountRear != undefined) {		  
				 ...
				 if(selected_qty >= 4){
					...
					jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set4price').removeClass('strike-price');
					jQuery(this).parents('.bundleBox').find('.rightbundle').find('.setfourprice').html(formatedSetofPrice);
					jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set4price').hide();
					jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set2price .set2').show();
			  ...
			  if(discountqty == undefined && discountqty == undefined) { 
                if(selected_qty == 4){
                  ...
                  if(selected_qty == 1){
                    jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set1price').hide();
					if(ruleDiscountRear != undefined){
						jQuery(this).parents('.bundleBox').find('.rightbundle').find('.set1price').show();
					}
                  }
                }
              }
            ...
            jQuery(this).parents('.bundleBox').find('.bundle-price-fr-rr').html(formatedFinalBundleTotal);
            jQuery(this).parents('.bundleBox').find('.bundle-total-qty').html('Set of '+total_qty+':');

            var isTabbyAvailableForBundle = jQuery(this).data('istabby');
            if(parseFloat(finalBundleTotal) >= tabbyfourday && parseFloat(finalBundleTotal) <= tabbyinstallmentmaxvalue && isTabbyAvailableForBundle == 'Yes'){
              jQuery(this).parents('.bundleBox').find('.tabbyinstaller').removeClass('hidetabbyinstaller');
            } else {
              jQuery(this).parents('.bundleBox').find('.tabbyinstaller').addClass('hidetabbyinstaller');
            }
   }); 
...
function getheight(widthValue,label,type)
{
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if(selectiondata != '') {
              jQuery('#tyre-selection .selectiondata').text(jQuery('#tyre-selection .selectiondata').text()+' / '+label);    
            }else{
              jQuery('#tyre-selection .selectiondata').text(label);    
            }
            jQuery('#tyre-selection .steps .width-step').addClass('active');
            jQuery('#tyre-selection .search-selection-wrap .search-title h4').text(HeightTxt);
            jQuery('.searchloader').hide();
        ...
        var widthValue = jQuery("#frontWidthValue-hidden").val();
         ...
         jQuery.ajax({
            ...
            success: function (data) {
              ...
              jQuery('#tyre-selection .search-selection-wrap .search-title h4').text(RimTxt);
              jQuery('.searchloader').hide();
  ...
  jQuery("#rearWidthLabel-hidden").val(label);
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            jQuery('.rear-tyre-selection').show();
            var rearselectiondata = jQuery('#tyre-selection .rearselectiondata').text();
                        if(rearselectiondata != '') {
                            jQuery('#tyre-selection .rearselectiondata').text(jQuery('#tyre-selection .rearselectiondata').text()+' / '+label);    
                        }else{
...
function selectRim(rimValue,label)
 {  
  ...
  jQuery('.frontRimLabel').removeAttr('disabled');
    ...
    jQuery(".frontRimLabel").text(label);
  jQuery('.front-width_sub-block, .front-height_sub-block, .front-rim_sub-block').hide();
  jQuery('#tyre-selection .confirm_sub-block').show();
  
  var selectiondata=jQuery('#tyre-selection .selectiondata').text();
    if(selectiondata != '') {
        jQuery('#tyre-selection .selectiondata').text(jQuery('#tyre-selection .selectiondata').text()+' R '+label);    
    }else{
        jQuery('#tyre-selection .selectiondata').text(label);    
    }
    ...
    if(jQuery('.rear-tyre-selection').is(':visible')) { 
        ...
        jQuery("#tyre-selection .selectiondata").text(widthLabel+' / '+profileLabel+' / '+label);    
    jQuery('.backrim').hide();    
    jQuery('.backfromrim').show();    
    }
...
function getmodel(value,label)
{
   ...
   jQuery(".mobile-tyre-search-finder #search-by-vehicle #vehicle_make").text(label);
   ...
   jQuery('#input-search').val('');
        ...
        jQuery.ajax({
          ...
          var vehicleselectiondata = jQuery('#vehicle-selection .vehicleselectiondata').text();
                    if(vehicleselectiondata != '') {
                        jQuery('#vehicle-selection .vehicleselectiondata').text(jQuery('#vehicle-selection .vehicleselectiondata').text()+' / '+label);    
                    }else{
                        jQuery('#vehicle-selection .vehicleselectiondata').text(label);    
                    }
          jQuery('.searchloader').hide();
          jQuery('#vehicle-selection .search-selection-wrap .search-title h4').text(ModelTxt);
            }
        });
}
...
function getyear(value,label)
{
    ...
    jQuery(".mobile-tyre-search-finder #search-by-vehicle #vehicle_model").text(label);
    ...
    jQuery('#input-search').val('');
        ...
        jQuery.ajax({
          ...
          jQuery('.searchloader').hide();
          jQuery('#vehicle-selection .search-selection-wrap .search-title h4').text(YearTxt);
            }
        });
}
...
function getengine(value,label)
{
    ...
    jQuery(".mobile-tyre-search-finder #search-by-vehicle #vehicle_year").text(label);
    jQuery("#vehicle_year_hidden").val(value);
        ...
        jQuery.ajax({
          ...
          jQuery('.vehicle_finder_modal_allcontent').hide();
          jQuery('.brand_sub-block, .vehiclemodel_sub-block, .vehicleyear_sub-block').hide();
          jQuery('.vehicleengine_sub-block').show();
          jQuery('.vehicleengine_sub-block ul').html(enginesTyre);
          jQuery('.vehicleengine_sub-block ul').append("<li id='last-note'><span>"+VehicleFinderLastPopupNote+"</span></li>");
                    var vehicleselectiondata = jQuery('#vehicle-selection .vehicleselectiondata').text();
          if(vehicleselectiondata != '') {
            jQuery('#vehicle-selection .vehicleselectiondata').text(jQuery('#vehicle-selection .vehicleselectiondata').text()+' / '+label);    
          }else{
          ...
          jQuery('.searchloader').hide();
          jQuery('#vehicle-selection .search-selection-wrap .search-title h4').text(TyreSizeTxt);
            }
        });
}
...
function getmodifications(value,label)
{
    ...
    jQuery(".mobile-tyre-search-finder #search-by-vehicle #vehicle_year").text(value);
    jQuery("#vehicle_year_hidden").val(value);
	...
	jQuery.ajax({
		...
		success: function (data) {
			 ...
			 // jQuery('.vehiclemodification_sub-block ul').append("<li id='last-note'><span>"+VehicleFinderLastPopupNote+"</span></li>");
			  ...
			  jQuery('.searchloader').hide();
			  jQuery('#vehicle-selection .search-selection-wrap .search-title h4').text(EngineTxt);
...
function getTyreSizes(value,label)
{
	...
	jQuery.ajax({
		...
		success: function (data) {
			...
			var enginesTyre=data.enginesTyre.join("");
			jQuery('.vehicle_finder_modal_allcontent').hide();
			jQuery('.brand_sub-block, .vehiclemodel_sub-block, .vehicleyear_sub-block, .vehiclemodification_sub-block').hide();
            jQuery('.vehicleengine_sub-block').show();
            jQuery('.vehicleengine_sub-block ul').html(enginesTyre);
            jQuery('.vehicleengine_sub-block ul').append("<li id='last-note'><span>"+VehicleFinderLastPopupNote+"</span></li>");
			jQuery('.vehicleallbackbutton').hide();
			jQuery('.vehiclebackengine').show();
			jQuery('.vehiclebackengine').attr('onclick', "vehcilegoback('vehiclemodification_sub-block','vehiclebackyear','vehicleyearimage','Engine')");
			var vehicleselectiondata = jQuery('#vehicle-selection .vehicleselectiondata').text();
			if(vehicleselectiondata != '') {
				var hasVehicleEngine = jQuery('#vehicle_engine_hidden').val();
				if(hasVehicleEngine){
					var contentText = makeLabel+' / '+ modelLabel+' / '+ year+' / '+label;
...
function goback(contentname,backbutton,type,searchbytype){
  ...
  jQuery(".tyre-size img").attr("src", $tyreImg205);
    ...
    jQuery('#tyre-selection .search-selection-wrap .search-title h4').text(WidthTxt);
  jQuery(".tyre-size img").attr("src", $tyreImgNormal);
    }
...
function resetSearchSuggestion(){
 ...
 function vehcilegoback(contentname,backbutton,roundimage,searchbytype){
    ...
    if(searchbytype == 'Engine' && contentname != 'vehiclemodification_sub-block') {
      jQuery('#vehicle-selection .vehicleselectiondata').text(vehicle_make+' / '+vehicle_model+' / '+vehicle_year);    
      jQuery('.vehiclestepper li').removeClass('active');
      jQuery('.vehiclestepper li').eq(3).addClass('active');
      jQuery('#vehicle-selection .search-selection-wrap .search-title h4').text('Engine');
    }
    ...
    if(searchbytype == 'Year') {
      jQuery('#vehicle-selection .vehicleselectiondata').text(vehicle_make+' / '+vehicle_model);    
      jQuery('.vehiclestepper li').removeClass('active');
    ...
    jQuery('#vehicle-selection .search-selection-wrap .search-title h4').text(MakeTxt);
    }
...
jQuery(".frontWidth, .frontHeight, .frontRim").click(function(event) {
    ...
    if(allWidthOptions_length <= 0 || allRearWidthOptions_length <= 0){
        ...
        jQuery.ajax({
          ...
          success: function (data) {
            jQuery('.tyre-size-search-selection .select2-tyre-size-search').html(data.response);
          },
      });
    }
    /* end get all width*/
  ...
  if(jQuery("#tyre-selection .confirm_sub-block").is(":visible")){
    ...
    //jQuery(".page-wrapper").animate({ scrollTop: 0 }, 0);
	jQuery("html, body").animate({ scrollTop: 0 }, 0);
    jQuery(".search-overlay").addClass("open");
});
 
jQuery(".vehicle_make, .vehicle_model, .vehicle_year").click(function(event) {
    jQuery("#vehicle-selection").fadeIn();
  if(jQuery("#vehicle-selection .brand_sub-block").is(":visible")){
    jQuery('.brand_sub-block').show();
    var searchType = jQuery(this).closest('#finder-vehicle-form').find('.search_type').val();
    var searchCatId = jQuery(this).closest('#finder-vehicle-form').find('.search_cat_id').val();
    if(searchType == 'vehicle-engine-search'){
      getTechDocMakeList(searchCatId);
      jQuery('#vehicle-selection .steps .year-step span.text').text(EngineTxt);
...
jQuery('.services-mobile .tyre-category').click(function(){
    ...
    jQuery('.site-header').attr('id', '');
    jQuery('.site-header').attr('id', 'default-version');
}); 
...
jQuery('.mobile-tyre-search-finder .close-searchpopup').click(function(){
    ...
    jQuery('.offer-services-cover').toggleClass('position-active');
    jQuery('.offer-services-cover').toggleClass('viewport-height');
  var imgClass = jQuery('.offers-banner-mobile .offer-slider .owl-item.active').find("img").attr('class');
  var defaultLogo = getMediaUrl+"images/logo/Logo.svg";
  jQuery('.site-header').attr('id', '');
    jQuery('.site-header').attr('id', imgClass);
    jQuery(".logo img").attr("src", defaultLogo);
});
...
jQuery('.services-list .row').each(function(){
    ...
    if (jQuery(this).find('.col').length > max) {
        jQuery(this).find('.col:gt('+max+')').addClass('hide').end().append('<div class="col-12 toggle-icon"><span class="show_more"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M360.5 217.5l-152 143.1C203.9 365.8 197.9 368 192 368s-11.88-2.188-16.5-6.562L23.5 217.5C13.87 208.3 13.47 193.1 22.56 183.5C31.69 173.8 46.94 173.5 56.5 182.6L192 310.9l135.5-128.4c9.562-9.094 24.75-8.75 33.94 .9375C370.5 193.1 370.1 208.3 360.5 217.5z"/></svg></span></div>');
        jQuery('.toggle-icon').click( function(){
            jQuery(this).siblings(':gt('+max+')').toggleClass('hide');
            jQuery('.offer-services-cover').toggleClass('viewport-height');
            if ( jQuery('.show_more').length ) {
                jQuery(this).html('<span class="show_less"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M23.5 294.5l152-143.1C180.1 146.2 186.1 144 192 144s11.88 2.188 16.5 6.562l152 143.1c9.625 9.125 10.03 24.31 .9375 33.93c-9.125 9.688-24.38 10.03-33.94 .9375l-135.5-128.4l-135.5 128.4c-9.562 9.094-24.75 8.75-33.94-.9375C13.47 318.9 13.87 303.7 23.5 294.5z"/></svg></span>');
            } else {
                jQuery(this).html('<span class="show_more"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M360.5 217.5l-152 143.1C203.9 365.8 197.9 368 192 368s-11.88-2.188-16.5-6.562L23.5 217.5C13.87 208.3 13.47 193.1 22.56 183.5C31.69 173.8 46.94 173.5 56.5 182.6L192 310.9l135.5-128.4c9.562-9.094 24.75-8.75 33.94 .9375C370.5 193.1 370.1 208.3 360.5 217.5z"/></svg></span>');
            };
        });
    };
});
...
jQuery(".filter-option-search input").on("keyup", function() {
  ...
  jQuery(jQuery(this).closest('.filter-options-content').find('.item')).filter(function() {
...
jQuery(".confirm_sub-block #is_rear_checked2").click(function(event) {
	...
	if(jQuery("#is_rear_checked").prop('checked') == true || jQuery("#is_rear_checked_mobile").prop('checked') == true){
		var widthLabel = jQuery(".frontWidthLabel").eq(0).text();
		var profileLabel = jQuery(".frontHeightLabel").eq(0).text();
		jQuery("#tyre-selection .crnt-or-frnt").html(jQuery("#tyre-selection .crnt-or-frnt").html().replace(CurrentTyreSelectionTxt, FrontTyreSelectionTxt));
		jQuery('#tyre-selection .tyre_finder_allcontent').hide();
		jQuery('#tyre-selection .rear-width_sub-block').show();
	}else{
		jQuery('#rearWidthValue-hidden').val('');
