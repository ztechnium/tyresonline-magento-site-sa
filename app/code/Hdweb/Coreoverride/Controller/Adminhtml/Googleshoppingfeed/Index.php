...
namespace Hdweb\Coreoverride\Controller\Adminhtml\Googleshoppingfeed;
...
class Index extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
		...
		if($categoryIds != ''){
			...
			$responseRowData[] = array('id', 'title', 'description', 'price', 'condition', 'link', 'availability', 'status', 'image_link', 'brand', 'identifier_exists');
			foreach($collection as $productData){
				...
				$responseRowData[] = array($sku, $title, $description, $pricewithTax, $condition, $productLink, $availability, $status, $imageLink, $brand, $identifier_exists);
			}
			 if (count($responseRowData) > 1) {
				$this->_createcsvfile($responseRowData);
			}
			$this->messageManager->addSuccess(__('Google shopping feed generated successfully'));
		}else{
			$this->messageManager->addError(__('Google shopping feed category not configured'));
		}	
	...
	public function _createcsvfile($responseRow)
    {
        if (count($responseRow) > 0) {
            ...
            $csvMediapath = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'google_shopping/';
            if (!is_dir($csvMediapath)) {
                $csvMediapath = mkdir($csvMediapath, 0777, true);
				
            }

            $outputFile = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'google_shopping/' . "Tyresonline_Google_Shopping.csv";
            $handle = fopen($outputFile, 'w');
            foreach ($responseRow as $response) {
                fputcsv($handle, $response);
            }
			
			$file_name = "Tyresonline_Google_Shopping.csv";
