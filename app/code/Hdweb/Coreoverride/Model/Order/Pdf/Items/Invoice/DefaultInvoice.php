...
class DefaultInvoice extends \Magento\Sales\Model\Order\Pdf\Items\Invoice\DefaultInvoice
{
    ...
    public function draw()
    {
        ...
        // draw Product name
        $lines[0][] = [
                'text' => $this->string->split($this->prepareText((string)$item->getName()), 370, true, true),
                'feed' => 35
        ];

        /*start for sr no*/
        /*changed feed value for sku in invoice.php and this file for this
        and price feed value changed only in invoice.php*/
        /* $var_srno = $this->registry->registry('sr_no');
        if($var_srno!=0)
        {
            $new_sr = $var_srno + 1;
            $sr_no = $new_sr;
            $this->registry->unregister('sr_no');
        }
        else{
            $new_sr = 1;
        }
        $this->registry->register('sr_no', $new_sr); */
        /*end for sr no*/
