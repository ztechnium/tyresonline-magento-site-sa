...
class Gccerptosup extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($rfcEnable == 1) {
			...
			//echo '<pre>';print_r(count($response));die;
            if (count($response) > 0) {
				...
				//	$this->sendEmailNotification($subject, $expectedCount, $actualCount, $dateDifference, $currentDate, $actualDate);
					$this->_messageManager->addSuccess('RFC successfuly executeed');
					return $resultRedirect->setPath('tyadmin/rfc/manage');
				}				
