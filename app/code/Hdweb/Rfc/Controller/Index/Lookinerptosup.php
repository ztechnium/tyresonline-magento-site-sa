...
class Lookinerptosup extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($rfcEnable == 1) {
			...
			$rfc->setData('rfc_name', 'LOOKIN ERP To Supplier');
			...
			$countquery = "SELECT max(TOT_RECORDS) FROM [LOOKIN].[dbo].[WITMAST]";
            $stmt1 = $conn->prepare($countquery);
            $stmt1->execute();
            $expectedCounts = $stmt1->fetchAll(\PDO::FETCH_ASSOC);
			
			$expectedCount = 1000; // Need to be in store rfc configuration
			
			if(count($expectedCounts) > 0){
				$expectedCount = $expectedCounts[0]['computed'];
			}
			
			$responsequery = "SELECT * FROM [LOOKIN].[dbo].[WITMAST]";
            $stmtresponse  = $conn->prepare($responsequery);
            $stmtresponse->execute();
            $responseCount = $stmtresponse->fetchAll(\PDO::FETCH_ASSOC);
			
			
            $query = "SELECT * FROM [LOOKIN].[dbo].[WITMAST] WHERE [STOCK] > 0 order by WTRFDATE DESC";
            ...
            if (count($response) > 0) {
				...
				$actualCount = $responseCount; //count($response);
				$subject = '';
				...
				if($expectedCount > $actualCount){
					...
					$rfc->save();
					$this->_messageManager->addError('RFC not executed and alert email sent');
					...
					$responseRowData[] = array('Supplier Code', 'Item Code', 'Item Desc', 'Item Brand', 'Item Size', 'RunFlat', 'Item Year', 'Item Qty', 'Item Price', 'Item Price2', 'Item Origin', 'Type', 'Product ID', 'Product SKU', 'Product Qty', 'Product Price', 'Product Offer', 'Product Status', 'Status');
					/* Start Delete records from RFC Supplier Products Table */
					$sql = "DELETE FROM " . $rfcSupplierTable . " WHERE supplier_code = 'Lookin'";
					...
					foreach ($response as $data) {
						...
						if(!empty($productData)) {
							...
							$successcount++;
							$responseRowData[] = array($supplierCode, $dataItcode, $dataDescription, $dataBrand, $itemSize, $hasRunFlat, $dataYear, $dataQty, $itemPrice, $itemPrice1, $itemOrigin, $type, $webProductId, $webProductSku, $webProductQty, $webProductPrice, $webProductOffer, $webProductStatus, 'Success');
						}else{
							$failedcount++;
							$responseRowData[] = array($supplierCode, $dataItcode, $dataDescription, $dataBrand, $itemSize, $hasRunFlat, $dataYear, $dataQty, $itemPrice, $itemPrice1, $itemOrigin, $type, 'Product Not Found', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'Failed');
						}
    ...
    public function getWebproductdata($itemCode, $itemYear)
    {
        ...
        $lookin_codeIdSql    = "SELECT attribute_id FROM " . $eavAttributeTable . " WHERE attribute_code = 'lookin_code' AND entity_type_id = " . $catalogEntityId . " LIMIT 1";
        $lookin_codeIdResult = $connection->fetchCol($lookin_codeIdSql);
        $lookin_codeId       = $lookin_codeIdResult[0];
...
// $productCollectionFactory = $this->_objectManager->get('\Magento\Catalog\Model\ResourceModel\Product\CollectionFactory');
        ...
        // $collection->addAttributeToFilter('lookin_code', 10);
        // $collection->addAttributeToFilter('dot', 2017);
        // echo $collection->getSelect();exit;

        // $query = "SELECT 'e.*', IF(at_lookin_code.value_id > 0, at_lookin_code.value, at_lookin_code_default.value) AS lookin_code, 'at_dot.value' AS 'dot' FROM `catalog_product_entity` AS `e` INNER JOIN `catalog_product_entity_varchar` AS `at_lookin_code_default` ON (`at_lookin_code_default`.`entity_id` = `e`.`entity_id`) AND (`at_lookin_code_default`.`attribute_id` = '502') AND `at_lookin_code_default`.`store_id` = 0 LEFT JOIN `catalog_product_entity_varchar` AS `at_lookin_code` ON (`at_lookin_code`.`entity_id` = `e`.`entity_id`) AND (`at_lookin_code`.`attribute_id` = '502') AND (`at_lookin_code`.`store_id` = 2) INNER JOIN `catalog_product_entity_int` AS `at_dot` ON (`at_dot`.`entity_id` = `e`.`entity_id`) AND (`at_dot`.`attribute_id` = '194') AND (`at_dot`.`store_id` = 0) WHERE (IF(at_lookin_code.value_id > 0, at_lookin_code.value, at_lookin_code_default.value) = '10') AND (at_dot.value = '2017')";
        ...
        // $result = $connection->fetchAll($query); // gives associated array, table fields as key in array.
        // echo "<pre>";
        // print_r($result);exit;

        $lookattribute_id = $lookin_codeId;
        $yearattribute_id = $yearattrId;

        $query = "SELECT e.*,at_lookin_code_default.value as 'lookattr',at_dot.value as 'dotattr' FROM catalog_product_entity AS e INNER JOIN catalog_product_entity_varchar AS at_lookin_code_default ON at_lookin_code_default.entity_id = e.entity_id
            AND at_lookin_code_default.attribute_id =" . $lookattribute_id . " INNER JOIN catalog_product_entity_int AS at_dot ON at_dot.entity_id= e.entity_id AND at_dot.attribute_id = " . $yearattribute_id . " WHERE at_lookin_code_default.value = " . $itemCode . " AND  at_dot.value = " . $yearOptionValueId . " ";
	...
	public function _createcsvfile($responseRow, $rfcid)
    {
        if (count($responseRow) > 0) {
            ...
            $outputFile = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . 'rfcfiles/' . "LOOKIN_ERP_To_Supplier_" . $this->getTodaysDate() . ".csv";
            ...
            $rfc = $this->_objectManager->create('Hdweb\Rfc\Model\Rfc')->load($rfcid);
            $rfc->setData('rfc_manual_path', "LOOKIN_ERP_To_Supplier_" . $this->getTodaysDate() . ".csv");
            $rfc->save();
        }
    }
